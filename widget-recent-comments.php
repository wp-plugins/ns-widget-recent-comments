...
<?php
	...
	function widget( $args, $instance ) {
		...
		if ( isset( $cache[$args['widget_id']] ) ) {
 		...
 		$output = '';
 		$title = apply_filters('widget_title', empty($instance['title']) ? __('Recent Comments', 'netscripter') : $instance['title']);
		...
		if ( $comments ) {
			foreach ( (array) $comments as $comment) {
				...
				$output .=  sprintf(__('%1$s on </div><li class="ns-comment"> %2$s', 'netscripter'), get_comment_author(), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_the_title($comment->comment_post_ID) . '</a>');
...
?>
		<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'netscripter'); ?></label>
		<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>

		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of comments to show:', 'netscripter'); ?></label>
		<input id="<?php echo $this->get_field_id('number'); ?>" name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>
		
		<p><label for="<?php echo $this->get_field_id('size'); ?>"><?php _e('Avatar size:', 'netscripter'); ?></label>
